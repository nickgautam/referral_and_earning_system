...
exports.earning_view = async (req, res) => {
    ...
    let get_data = await earning.find({ user_id: _id }).populate('user_id', 'name earnings').populate("buyer_id", "name")
...
exports.get_my_earning_details = async (req, res) => {
    try {
        let get_data = await earning.find().populate('user_id', 'name earnings').populate("buyer_id", "name")
